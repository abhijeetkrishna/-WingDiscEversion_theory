...
def get_fit_lambda(pos_vector, DV_present = True, volume_conservation = True,
                   theta_DV = 0, theta_max = np.pi/2, theta_ref = None,
    ...
    if DV_present and np.abs(pos_vector[0]) <= r*np.sin(theta_DV/2) :
        ...
        lambda_anisotropic = inDV_lambda_anisotropic_obj(abs_phi/theta_ref) 
        lambda_isotropic = inDV_lambda_isotropic_obj(abs_phi/theta_ref) 
        ...
        if (lambda_height_obj is None) and (volume_conservation):
            ...
            lambda_height = inDV_lambda_height_obj(abs_phi/theta_ref)
        ...
        #get lambda coefficients
        lambda_anisotropic = lambda_anisotropic_obj((theta - theta_DV/2)/(theta_ref - theta_DV/2)) #in-surface distance = r*theta
        lambda_isotropic = lambda_isotropic_obj((theta - theta_DV/2)/(theta_ref - theta_DV/2))
        lambda_thetaTheta = lambda_isotropic*lambda_anisotropic
        lambda_phiPhi = lambda_isotropic*(lambda_anisotropic**(-1))
        if (lambda_height_obj is None) and (volume_conservation):
            lambda_rr = 1/((lambda_isotropic)**(2*nu))
        else:
            lambda_height = lambda_height_obj((theta - theta_DV/2)/(theta_ref - theta_DV/2))
            lambda_rr = lambda_height
...
def get_fit_lambda_for_mesh_from_data(row, DV_present = True,
                   lambda_anisotropic_obj = None, lambda_isotropic_obj = None,
                   inDV_lambda_anisotropic_obj = None, inDV_lambda_isotropic_obj = None,
                   #lambda_height_obj = None, inDV_lambda_height_obj = None,
                   ):
